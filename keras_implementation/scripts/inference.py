...
def parse_args():
    ...
    parser.add_argument('--low_psnr_percentile', default=0.2, type=int, help='Lower percentile for PSNR value used to '
                                                                             'send image patches to low-noise model')
    parser.add_argument('--upper_psnr_percentile', default=0.8, type=int, help='Upper percentile for PSNR value used '
...
def denoise_image_by_patches_no_cross_reference(y, file_name, set_name, original_mean, original_std, save_patches=True,
                                                single_denoiser=False, model_dict=None):
    """
...
def denoise_image_by_patches(y, file_name, set_name, original_mean, original_std, save_patches=True,
    ...
    for i in range(0, len(y[0]), 40):
        for j in range(0, len(y[1]), 40):
            ...
            # TODO: FIGURE OUT WHETHER TO COMPARE TO Y (WITH NOISE) OR X (WITHOUT NOISE) ##################
...
def modified_main(args):
    ...
    for set_name in args.set_names:
        ...
        # Get the upper and lower psnr percentiles
        low_psnr_thresh, upper_psnr_thresh = data_generator.get_lower_and_upper_percentile_psnr(
            os.path.join(args.set_dir, set_name), lower_percentile=args.low_psnr_percentile,
            upper_percentile=args.upper_psnr_percentile)
